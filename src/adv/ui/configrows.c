/* Persona 1 (JP) - the config page's rows.  ADV only.
    ...
    if (InputCheckAcceptA(0)) {
        if (g_menu->cfg_row.cur == ROW_AUTO_BATTLE) {
            ...
            for (i = 0; i <= g_party_last; i++) {
                ...
                TileMapWriteRow(D_800B189C, AT(g_tilemap1, 3 + i * 2, 11), 0,
                                4);
                TileMapWriteRow(D_800B189C + 4, AT(g_tilemap1, 3 + i * 2, 16),
                                0, 4);
                TileMapWriteRow(D_800B189C + 8, AT(g_tilemap1, 3 + i * 2, 21),
                                0, 4);
            }
