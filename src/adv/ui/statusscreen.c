/* Persona 1 (JP) - the status menu's member page.  ADV only.
 *   0x80071F68 MenuStatusOpen   0x800720EC MenuStatusSelect
 *   0x80072A08 MenuStatusView
 *
 * Picking a member opens their page: the member's face is read off the disc
 * (AdvResolveSceneLoc kind 3, five sectors) and queued into VRAM with one
 * palette entry patched, the page is laid out over the two character-map
 * layers, and a message names the member. The page has three stops the
 * cursor scrolls between, 8 lines a frame.
 */
...
extern void  StatusDrawMain(u_char slot);
...
void MenuStatusSelect(void)
{
    ...
    if (InputCheckAcceptA(1)) {
        ...
        *AT(g_tilemap1, 27, 23) = GLYPH_SLASH;
        ...
        SlotInitTagged(D_800B1E98, 0x2E, 0x24, 0x48, 0xC6);
