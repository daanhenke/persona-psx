...
#define ARCANA_MARKS()                                                            g_slot_cur = &g_slots[PAGE_MARK_SLOT];                                        if (g_arcana_top == 0) {                                                          g_slot_cur->attr |= SLOT_ATTR_HIDE;                                       } else {                                                                          g_slot_cur->attr &= ~SLOT_ATTR_HIDE;                                      }                                                                             g_slot_cur = &g_slots[PAGE_MARK_SLOT + 1];                                    if (g_arcana_top == ARCANA_TOP_MAX) {                                             g_slot_cur->attr |= SLOT_ATTR_HIDE;                                       } else {                                                                          g_slot_cur->attr &= ~SLOT_ATTR_HIDE;                                      }

/* Two arcana a row; the grid scrolls by rows of 12 lines, at most 4 down. */
#define ARCANA_ROW_H   12
#define ARCANA_TOP_MAX 4
#define ARCANA_AT_CURSOR()                                                        (g_arcana_top * 2 + g_menu->arcana_col.cur + g_menu->arcana_row.cur * 2)
...
extern u_char  D_8009A5DC[];
extern u_char  D_8009ABDC[];
extern u_char  D_8009CAFC[];
...
/* The screen's archive, preloaded by the field. */
...
extern u_char  D_8009AC3C[];
extern u_char  FlagsCollectGroup(u_char group);
extern void    DrawPersonaList(void);
/* Both declared returning int here: the image tests the results unnarrowed. */
extern int     PageScrollValue(short *value, short lo, short hi, short step);
...
/* The screen, from the field: the archive preloaded at 0x80180000 carries
   the screen's eight images, which go to VRAM; the three character-map
   layers are set up over the work area's tile maps, the menu cursors reset,
   and the arcana grid opened. The screen runs between a fade up and a fade
   down, and its four sound sequences are closed after it. */
...
void ArcanaGridOpen(void)
{
    ...
    SlotInitTagged(D_8009AC3C, 0x2C, 0x24, 0xE7, 0xE);
...
/* A frame of the grid: the rows scroll 12 lines at a time with the page
   buttons or past either end, the cursor moves between the two columns,
   and a new arcana under the cursor swaps the help line and the persona
   list. A picks from the list; B leaves. */
...
/* Lays the page out in the two character-map layers: the window and its
   box, the page's fixed text, the stat bars and the resistance rows; the
   page sprites go to the stop the page cursor is on. */
void PersonaDataLayout(void)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        TileMapWriteBar(AT(g_tilemap0, 24 + i, 2), 0x19);
        {
            ...
            TileMapWriteRow(&D_8009B330[i * 3], dst, 0, 3);
        }
    }
    TileMapWriteRow(D_8009A5DC, AT(g_tilemap1, 29, 20), 0x1AE, 4);
    ...
    SlotInitTagged(D_8009ABDC, 0x2E, 0x24, 0x54, 0xC6);
    SlotInitTagged(D_8009CAFC, 0x1D, 0x20, 0x5C, 0xC8);
    SlotClear(1);
    SlotClear(PICK_CURSOR_SLOT);
    ...
    g_cam_y = i;
    g_map_scroll_y = i;
    SlotSetPos(0, 0x52, 0x50, 0x48 - g_cam_y);
...
/* Fills the page in for persona `id`: level, HP and SP, the two stats in
   the corner, the five stats and their bars, arcana and class, the five
   spells, the resistance line, and the name and arcana cells. */
    ...
    short id;
{
    ...
    i = FormatDecimal(g_persona_data[id].level, g_hud_digits, 2);
    TileMapWriteRowRev(g_hud_digits, AT(g_tilemap1, 5, 25), GLYPH_DIGIT0, i);

    *AT(g_tilemap1, 7, 24) = GLYPH_SLASH;
    i = FormatDecimal(g_persona_data[id].hp, g_hud_digits, 3);
    TileMapWriteRowRev(g_hud_digits, AT(g_tilemap1, 7, 23), GLYPH_DIGIT0, i);
    TileMapWriteRowRev(g_hud_digits, AT(g_tilemap1, 7, 27), GLYPH_DIGIT0, i);
    *AT(g_tilemap1, 8, 24) = GLYPH_SLASH;
    i = FormatDecimal(g_persona_data[id].sp, g_hud_digits, 3);
    ...
    TileMapWriteRowRev(g_hud_digits, AT(g_tilemap1, 24, 17), GLYPH_DIGIT0, i);
    i = FormatDecimal(g_persona_data[id].stat[1], g_hud_digits, 2);
    TileMapWriteRowRev(g_hud_digits, AT(g_tilemap1, 25, 17), GLYPH_DIGIT0, i);
    i = FormatDecimal(g_persona_data[id].stat[2], g_hud_digits, 2);
    TileMapWriteRowRev(g_hud_digits, AT(g_tilemap1, 26, 17), GLYPH_DIGIT0, i);
    i = FormatDecimal(g_persona_data[id].stat[3], g_hud_digits, 2);
    TileMapWriteRowRev(g_hud_digits, AT(g_tilemap1, 27, 17), GLYPH_DIGIT0, i);
    i = FormatDecimal(g_persona_data[id].stat[4], g_hud_digits, 2);
    TileMapWriteRowRev(g_hud_digits, AT(g_tilemap1, 28, 17), GLYPH_DIGIT0, i);
