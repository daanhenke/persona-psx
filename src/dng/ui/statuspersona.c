/* Persona 1 (JP) - the status menu's Persona pages.  DNG only.
 *   0x8007E324 StatusPersonaPick     0x8007E628 StatusPersonaLayout
 *   0x8007E9F0 StatusPersonaPreview  0x8007EA80 StatusPersonaDraw
 *   0x8007EE48 StatusPersonaNames    0x8007EF08 StatusPersonaView
 *   0x8007F364 StatusStockPick       0x8007F6B8 StatusStockReleasePick
 *   0x8007F950 StatusStockReleaseConfirm  0x8007FB28 StatusStockView
 *
 * The field's copy of ADV's status menu Persona pages
 * (src/adv/ui/statuspersona.c), built against the field's own message
 * stepper. Four of its frames read a portrait off the disc: the Persona is
 * narrowed down to its file (AdvResolveSceneLoc kind 5 for a member's
 * Persona, kind 4 for a stock one), its sectors streamed to the staging
 * buffer while the frame loop keeps running, and the TIM behind the entry's
 * header queued into VRAM.
 */
...
extern void   StatusPageDraw(short member);
...
extern u_char D_8009A3C0[];
extern u_char D_8009A3F4[];
extern u_char D_8009A524[];
extern u_char D_8009A5DC[];
extern u_char D_8009A6CF[];
...
void StatusPersonaLayout(void)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        TileMapWriteRow(&D_8009B330[i * 3], AT(g_tilemap1, row, 12), 0, 3);
    }
    ...
    SlotInitTagged(D_8009A3F4, 0, 8, 0x40, 0x34);
    SlotInitTagged(D_8009ABDC, 0x2E, 0x24, 0x54, 0xC6);
    SlotInitTagged(D_8009A3C0, PAGE_TOP_SLOT, 0x20, 0x5C, 0xCC);
...
void StatusStockView(void)
{
    ...
    if ((g_cam_y == 0 || g_cam_y == PAGE_LOW) &&
        ...
        TileMapWriteRow(D_8009A524, AT(g_tilemap1, 13, 2), 0x285, 6);
