...
void FormationPlaceMember(void)
{
    ...
    /* The image gives `cell` the first saved register: flow weights each
       reference by loop depth, and only this nesting reaches its priority
       without changing the code. */
    do {
        cell = g_menu->grid[0].cur * GRID_W;
        do {
            cell += g_menu->grid[1].cur;
        } while (0);
    } while (0);
