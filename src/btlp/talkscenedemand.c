...
#define DEMAND_KINDS BTL_DEMAND_KINDS
...
{
    const u_char* p;
    /* The first 16 bytes of this 48-byte scratch area are untouched. */
    u_short               items[DEMAND_ITEM_COUNT + 8];
    BtlOffer*             offer;
    BtlActor*             e;
    const BtlDemandScene* base;
    int                   answer;
    int                   gauge;
    int                   weight;
    u_long                offset;
    ...
    base  = (const BtlDemandScene*)TALK_BASE;
    ...
    switch (g_btl_talk_stage[g_btl_talk_depth - 1])
    {
        case 1:
            ...
            memmove((u_char*)g_btl_demand_odds,
                    (u_char*)g_btl_demand_odds_by_kind[g_persona_data[g_btl_offer[g_btl_offer_slot].persona].arcana],
            ...
            for (i = 0; i < DEMAND_KINDS; i++)
            {
                switch (i)
                {
                    case DEMAND_MONEY:
                        g_btl_demand_money =
                            g_persona_data[g_btl_offer[g_btl_offer_slot].persona].price * ((rand() % 3) * 5 + 0x50) / 100 * 2 + 1;
                        if (g_money < g_btl_demand_money && (g_btl_talk_asked & ASKED_MONEY) != 0)
                        {
                        ...
                        break;
                    case DEMAND_CHARM:
                        if ((u_short)BtlItemSlot(DEMAND_CHARM_SLOT) == 1 && (g_btl_talk_asked & ASKED_CHARM) != 0)
                        {
                    ...
                    case DEMAND_ITEM:
                        ...
                        for (i = 0; i < DEMAND_ITEM_COUNT; i++)
                        {
                            ...
                            {
                                items[g_btl_demand_item + 8] = item;
                                g_btl_demand_item++;
                            }
                        }
            ...
            {
                if (g_btl_demand_odds[g_btl_demand_kind] != 0 && g_btl_demand_odds[g_btl_demand_kind] >= weight)
                {
                    break;
                }
                g_btl_demand_kind++;
            ...
            switch (g_btl_demand_kind)
            {
                ...
                case DEMAND_ITEM:
                    n                 = rand();
                    g_btl_demand_item = items[n % g_btl_demand_item + 8];
            ...
            {
                i = (g_btl_actors[g_btl_actor_slot].c.key - 1) * 4 + 2;
            }
            else
            {
                i = (g_btl_actors[g_btl_actor_slot].c.key - 1) * 4;
            }
        ...
        case 2:
            ...
            switch (g_btl_demand_kind)
            {
                ...
                case DEMAND_ITEM:
                    ...
                    {
                        i = 2;
                        BtlItemRemove((short)g_btl_demand_item);
                    }
                    else
                    {
                        i = rand() % 2 + 5;
        ...
        take_line:
            ...
            if (g_btl_demand_kind == DEMAND_BLOOD && g_btl_demand_answer == DEMAND_YES)
            {
                BtlTalkPerform();
            }
            ...
            {
                if ((((short)g_btl_panel_gauges >> gauge) & 1) != 0 && (g_btl_offer[g_btl_offer_slot].kinds & (1 << gauge)) != 0 && weight > TALK_VOICE_MIN - 1)
                {
                    BtlSeqRun();
                    BtlEndTalking();
                    g_btl_talk_depth--;
