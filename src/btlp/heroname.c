...
/* Picks a negotiation back up without resetting it: the round's contacts are
   folded into the moods, the offers are rebuilt from whoever is still there,
   and the flags are cleared. BtlResetTalk is the version that starts one. */
void BtlTalkResume(void)
{
    BtlOfferApplyScores();
    BtlBuildOffers();
    g_btl_talk_flags = 0;
