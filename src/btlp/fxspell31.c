 ...
 * to a routine further down the block and answers what that builds. The target
...
/* Still assembly: what actually opens the records. */
extern BtlObj *func_800B9D58(int slot);

BtlObj *BtlFxStart31(void)
{
    return func_800B9D58(g_btl_fx_target);
}
