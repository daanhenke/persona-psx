...
void BtlMenuUpdate(void)
{
    ...
    case BTL_MENU_LIVE:
        if ((g_btl_key_up & BtlInputKeys()) != 0) {
        ...
        if ((g_btl_key_down & BtlInputKeys()) != 0) {
            BtlSePlay(MENU_SE_BANK, MENU_SE_MOVE);
        ...
        *choice = BTL_MENU_WAIT;
        ...
        BtlCursorPlace(g_btl_menu_cells[g_btl_menu_index].x * MENU_CURSOR_STEP
                           + MENU_CURSOR_X0,
...
#else
INCLUDE_ASM("btlp/nonmatchings/menuupdate", BtlMenuUpdate);
#endif
