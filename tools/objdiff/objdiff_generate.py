...
def _create_config():
    ...
    # Where the objects being measured live. They are built NON_MATCHING, so
    # they are kept out of the ordinary build tree - see NM_BUILD_BASE_DIR in
    # the Makefile.
    parser.add_argument("--build-base", type = str, default = "build")
...
def _determine_categories(path: Path, config) -> tuple[UnitMetadata, str]:
    ...
    if game_version != "ALL":
        ...
        return UnitMetadata(categories, f"src/{fixed_ext}.c"), fixed_ext, re.search(r"/(JP1|US)/", str(path))[0]
...
def main():
    ...
    for file in expected_objects:
        ...
        if game_version != "ALL":
            base_path = f"{build_base}/{game_version}/src/{input_path}.c.o"
        else:
            base_path = f"{build_base}{processed_path[2]}src/{input_path}.c.o"
    ...
    with (Path(config["output"])).open("w") as json_file:
        json.dump(asdict(Config(True, False, build_method, ["build", f"GAME_VERSION={game_version}", "NON_MATCHING=1", "SKIP_ASM=1", "OBJDIFF_FIX=1", f"BUILD_BASE_DIR={build_base}"] if build_method != "ninja" and game_version != "ALL" else None, units, categories)), json_file, indent=2)
