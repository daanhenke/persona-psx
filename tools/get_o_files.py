...
def get_o_target(yaml_file, release, compile_asm_tu, build_base = None):
    ...
    with open(yaml_path, 'r') as file:
        ...
        # The yaml says build/<release>, which is where an ordinary build goes.
        # A build asked to go elsewhere - the NON_MATCHING one the progress
        # report measures - has to be told, or the object list it gets back
        # names files nothing in that tree will produce.
        if build_base:
            build_path = f"{build_base}/{release}"
                        ...
                        for splits in segment.get('subsegments'):
                            try:
                                match splits[1]:
                                    ...
                                    case "hasm":
                                        splits_c.append(f"{src_path}/{splits[2]}.s.o")
                                    case ".rodata" | ".data" | ".sdata" | ".bss" | ".sbss" | ".rdata":
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 tools/get_o_files.py <config_file>.yaml <release>"
              " [compile_asm_tu] [build_base]")
        sys.exit(1)
    
    yaml_file      = sys.argv[1]
    release        = sys.argv[2]
    compile_asm_tu = sys.argv[3] if len(sys.argv) > 3 else 0
    build_base     = sys.argv[4] if len(sys.argv) > 4 else None
    target_path = get_o_target(yaml_file, release, compile_asm_tu, build_base)
    print(*target_path)
