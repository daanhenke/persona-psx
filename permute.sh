...
if [[ "$ASM_FUNCTION" == *"/main/"* || "$ASM_FUNCTION" == *"/atlus/"* || "$ASM_FUNCTION" == *"/open/"* ]]; then
    G_OPTION="-G8"
else
    G_OPTION="-G0"
fi
...
echo "Creating compile.sh file"
{
    ...
    echo "cd $SCRIPT_DIR"
    echo ''
    echo 'mipsel-linux-gnu-cpp -P -MMD -MP -MT "$INPUT_I" -MF "$INPUT_D" -Iinclude -Iinclude/psyq -Iinclude/decomp -I build -D_LANGUAGE_C -DUSE_INCLUDE_ASM -DNON_MATCHING -P -MMD -MP -undef -Wall -lang-c -nostdinc -o "$INPUT_I" "$INPUT"'
    echo "tools/bin/gcc-2.6.0/cc1 -O2 $G_OPTION -mips1 -mcpu=3000 -w -funsigned-char -fpeephole -ffunction-cse -fpcc-struct-return -fcommon -fverbose-asm -msoft-float -mgas -fgnu-linker -quiet -o \"\$INPUT_S\" \"\$INPUT_I\""
    echo "python3 tools/maspsx/maspsx.py --aspsx-version=2.34 --run-assembler --expand-div -EL -Iinclude -Iinclude/psyq -I build -O2 $G_OPTION -march=r3000 -mtune=r3000 -no-pad-sections -o \"\$OUTPUT\" \"\$INPUT_S\" < /dev/null"
...
gcc -E -P -Iinclude -Iinclude/decomp -Iinclude/psyq -DPERMUTER -DNON_MATCHING $C_FUNCTION > permuter/$FUNCTION_NAME/base.c

echo "Assembling the ASM function permuter/$FUNCTION_NAME/target.o"
mipsel-linux-gnu-as -EL -Iinclude -Iinclude/decomp -Iinclude/psyq -I build -O2 -march=r3000 -mtune=r3000 -no-pad-sections $G_OPTION -o permuter/$FUNCTION_NAME/target.o permuter/$FUNCTION_NAME/target.s
