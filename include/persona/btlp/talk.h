...
/* The pack itself, always a bare address: 0x801C0000 has nothing in its low
   half, so one `lui` does it, and the image shares that `lui` between the
   address and the first word read out of it, which a symbol cannot do. The
   sym file bans the base word so splat does not name it either. The words
   after it are read with a %hi/%lo pair of their own, so they have names:
   where the choice rows and the acts that go with them begin, and how long
   the whole pack is. */
